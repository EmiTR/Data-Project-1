...
Sample data: 
```{sql connection=con}
create table rewe_web.onetable_sampleid as
select * 
from rewe_web.onetable tablesample BERNOULLI(10)
...
*alread done
```{sql connection=con}
delete from rewe_web.onetable_sampleid where visit_binary like '0';
delete from rewe_web.onetable_sampleid where order_binary like '1' and revenue like '0';
delete from rewe_web.onetable_sampleid where pageview like '0' and visit_number like '1';
delete from rewe_web.onetable_sampleid where tracking_code like '';
```

Re-formating
*alread done
```{sql connection=con}
create table rewe_web.onetable_format as
select
...
from rewe_web.onetable_sampleid;
```

3.2, Exploring
Pull the table into R to do some quick summary 
```{r}
onetable_format <- dbReadTable(con, c("rewe_web","onetable_format"))
```
```{r}
...
IQR = Q_3 - Q_1
```{sql connection=con}
delete from rewe_web.onetable_format where visit_number > (6 + 1.5*5);
```
```{sql connection=con}
delete from rewe_web.onetable_format where pageview > (4 + 1.5*3 + 50);
```
3.4 Visualising
...
```{sql connection=con}
select * from rewe_web.onetable_model limit 10;
```
```{sql connection=con}
select visitor_id,sum()  from rewe_web.onetable_model;
```
*0207 nhung doan sau nay la do lam viec o nha

```{r}
onetable_training1 <- dbReadTable(con, c("rewe_web","onetable_training1"))
```
```{r}
onetable_test1 <- dbReadTable(con, c("rewe_web","onetable_test1"))
```
...
where visitor_id in (select distinct visitor_id from rewe_web.onetable);
```{sql connection=con}
create table rewe_web.onetable_training1 as
select * 
from rewe_web.onetable_model tablesample BERNOULLI(75)
where visitor_id in (select distinct visitor_id from rewe_web.onetable_model);
```
```{sql connection=con}
create table rewe_web.onetable_test1 as
select * 
from rewe_web.onetable_model 
where visitor_id in (select distinct visitor_id from rewe_web.onetable_model) and visitor_id not in (select distinct visitor_id from rewe_web.onetable_training1) ;
```
cac buoc tiep theo:
```{r}
...
```{sql connection=con}
select count(distinct visitor_id) from rewe_web.onetable_training1;
```
```{sql connection=con}
select count(distinct visitor_id) from rewe_web.onetable_training1 where order_binary = 1;
```
```{sql connection=con}
select visitor_id, sum(order_binary) from rewe_web.onetable_training1 where order_binary = 1 group by visitor_id order by sum(order_binary) desc;
```
